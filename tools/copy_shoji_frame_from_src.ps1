$src = 'd:\Minecraft\mods\mod workspace\sengoku attempt 4\src\main\resources\assets\sengoku\textures\block'
$dst = Join-Path $src 'shoji_frame'
if(-not (Test-Path -Path $dst)){
    New-Item -ItemType Directory -Path $dst | Out-Null
}
$files = Get-ChildItem -Path $src -Filter '*trapdoor*.png' -File -ErrorAction SilentlyContinue
